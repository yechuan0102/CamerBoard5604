...
{
	//SIU.PCR[0].R = 0x0203;//A12_D0 output/0x0100 input
	//SIU.PCR[1].R = 0x0203;//A13_D1
	//SIU.PCR[2].R = 0x0203;//A14_D2
	//SIU.PCR[3].R = 0x0203;//A15_D3
	SIU.PCR[37].R = 0x0203;	//PC5 RST
	SIU.PCR[43].R = 0x0203;	//PC11 DC
	//LCD_SCL=1;
	LCD_RST=0;
	delay_ms(1000);
	LCD_RST=1;
...
/*********************LCD显示base***************************/
void LCD_WrDat(BYTE data)//DC=1数据（XS128）
{
/*	BYTE i=8;
	//LCD_CS=1;;
	LCD_DC=1;
...
void LCD_WrCmd(BYTE cmd)  //输出cmd DC=0命令
{
/*	BYTE i=8;
	...
	while(i--)
	{
		if(cmd&0x80){LCD_SDA=1;}  //if cmd第一位!=0  PA1输出
		else{LCD_SDA=0;;}
		LCD_SCL=1;;
		asm("nop");;
			//asm("nop");             
...
0x84,0x8C,0x10,0x00,0x88,0xC8,0xE8,0xB9,0x9A,0x88,0xA8,0xCC,0xC8,0x00,
0x40,0x61,0x31,0x0C,0x60,0x1C,0x07,0x00,0x3F,0x00,0x3F,0x40,0x40,0x20,//流25
0x10,0x93,0x56,0x30,0x00,0xF4,0x94,0x94,0xFF,0x94,0x94,0xF4,0x00,0x00,
0x18,0x31,0x29,0x67,0x48,0x48,0x46,0x41,0x7F,0x41,0x46,0x58,0x40,0x40,//速26
0x00,0x08,0xF8,0x08,0x48,0xE9,0x4B,0x4C,0x48,0xE8,0x48,0x0C,0x08,0x00,
0x60,0x3C,0x03,0x40,0x40,0x6D,0x35,0x15,0x35,0x6D,0x44,0x40,0x40,0x00,//度27
...
void LCD_Rectangle(BYTE x1,BYTE y1,BYTE x2,BYTE y2,BYTE gif)
{
	...
	{
		LCD_WrDat(0x01<<(y1%8)); 			
		if(gif == 1) 	asm("nop");asm("nop");asm("nop");
	}  
	LCD_Set_Pos(x1,y2>>3);
  for(n=x1;n<=x2;n++)
	{
	...
	while(ch[ii] != '\0')
	{
  	...
  	while(F14x16_Idx[wm] > 127)
  	{
  		if(F14x16_Idx[wm] == ch[ii])
  		{
  			...
  			{
  				adder = (WORD)(wm * 14);	//avoid warming
  				break;
  			}
  		}
  		wm += 2;			
